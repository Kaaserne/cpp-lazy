...
//     LZ_CONSTEXPR_CXX_17
//     join_where_iterable(IterA iter_a, SA end_a, IterB iter_b, SB end_b, SelectorA a, SelectorB b, ResultSelector
//     result_selector) :
...
//  * `result_selector` if those are equal. The selector for a must be a function with a parameter of type =
//  `decltype(iterable_a[n])`.
//  * The selector for b must be a function with a parameter of type = `decltype(iterable_b[n])`. The selector for the result must
//  be
...
//  * @param result_selector A function that takes two parameters as its arguments. The value type of iterable `iterable_a` and
//  the
