...
// /**
//  * Converts an iterable into a chain_iterable, where one can chain iterators using dot operator
//  (.filter().map().select().any())
...
//     enumerate(const Arithmetic start = 0) const {
//         return chain(lz::enumerate(*this, start));
//     }
...
//         detail::cartesian_product_iterator<std::tuple<Iterator, iter_t<Iterables>...>, std::tuple<S,
//         sentinel_t<Iterables>...>>;
...
//     using flatten_iterator = detail::flatten_iterator<Iterator, S, detail::count_dims<std::iterator_traits<Iterator>>::value -
//     1>;
...
//     LZ_NODISCARD
//     LZ_CONSTEXPR_CXX_20 chain_iterable<detail::inclusive_scan_iterator<Iterator, S, detail::decay_t<T>,
//     detail::decay_t<BinaryOp>>> inclusive_scan(T&& init = {}, BinaryOp&& binary_op = {}) const {
...
//         return chain(lz::unique(*this, std::move(binary_predicate)));
//     }
...
//     LZ_NODISCARD LZ_CONSTEXPR_CXX_20
//     chain_iterable<detail::join_where_iterator<Iterator, S, iter_t<IterableB>, sentinel_t<IterableB>, SelectorA, SelectorB,
//     ResultSelector>> join_where(IterableB&& iterable_b, SelectorA selector_a, SelectorB selector_b, ResultSelector
//     result_selector) const {
//         return chain(lz::join_where(*this, iterable_b, std::move(selector_a), std::move(selector_b),
...
//     /**
//      * Checks if all of the elements meet the condition `unary_predicate`. `unary_predicate` must return a bool and take a
//      `value_type` as
//      * parameter.
...
//     /**
//      * Checks if any of the elements meet the condition `unary_predicate`. `unary_predicate` must return a bool and take a
//      `value_type` as
//      * parameter.
//      * @param unary_predicate The function that checks if an element meets a certain condition.
//      */
...
//     /**
//      * Checks if none of the elements meet the condition `unary_predicate`. `unary_predicate` must return a bool and take a
//      `value_type` as
//      * parameter.
//      * @param unary_predicate The function that checks if an element meets a certain condition.
//      */
