...
constexpr Iterator max_element(Iterator begin, S end, BinaryPredicate binary_predicate = {}) {
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_14 iter_t<Iterable> max_element(Iterable&& iterable, BinaryPredicate&& binary_predicate = {}) {
    ...
    return max_element(detail::begin(iterable), detail::end(iterable), std::forward<BinaryPredicate>(binary_predicate));
}
} // namespace lz
