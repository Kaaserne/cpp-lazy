...
LZ_CONSTEXPR_CXX_14 Iterator partition(Iterator begin, S end, UnaryPredicate&& unary_predicate) {
    if constexpr (detail::is_sentinel_v<Iterator, S>) {
        return detail::algorithm::partition(std::move(begin), std::move(end), std::forward<UnaryPredicate>(unary_predicate));
    }
    else {
        return std::partition(std::move(begin), std::move(end), std::forward<UnaryPredicate>(unary_predicate));
    }
}

/**
 * @brief Partitions the iterable in such a way that all elements that satisfy the unary_predicate come before all elements that
 * do not satisfy the unary_predicate. The relative order of the elements is not preserved.
 *
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_14 iter_t<Iterable> partition(Iterable&& iterable, UnaryPredicate&& unary_predicate) {
    return partition(detail::begin(iterable), detail::end(iterable), std::forward<UnaryPredicate>(unary_predicate));
}
