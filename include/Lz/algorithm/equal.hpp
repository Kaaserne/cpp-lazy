...
[[nodiscard]] constexpr bool equal(Iterator1 begin1, S1 end1, Iterator2 begin2, S2 end2, BinaryPredicate binary_predicate = {}) {
    ...
    return equal(std::move(begin1), std::move(end1), std::move(begin2), std::move(end2), std::move(binary_predicate));
}
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_14 bool equal(IterableA&& a, IterableB&& b, BinaryPredicate&& binary_predicate = {}) {
    ...
    using std::equal;
    return equal(detail::begin(a), detail::end(a), detail::begin(b), detail::end(b),
