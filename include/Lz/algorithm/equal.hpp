...
[[nodiscard]] constexpr bool equal(IterableA && a, IterableB && b, BinaryPredicate binary_predicate = {}) {
    ...
    using lz::equal; // TODO check if all these are necessary if they rely on lz:: equivs
    using std::equal;
    return equal(detail::begin(a), detail::end(a), detail::begin(b), detail::end(b), std::move(binary_predicate));
}
...
template<class IterableA, class IterableB, class BinaryPredicate = detail::equal_to>
LZ_NODISCARD LZ_CONSTEXPR_CXX_14
    detail::enable_if_t<detail::is_sized<IterableA>::value && detail::is_sized<IterableB>::value, bool>
