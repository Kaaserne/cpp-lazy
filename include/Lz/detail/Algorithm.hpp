...
template<class Iterator, class S, class UnaryPredicate>
enable_if<std::is_same<Iterator, S>::value, Iterator> find_last_if(Iterator begin, S end, UnaryPredicate unary_predicate) {
    std::reverse_iterator<Iterator> rbegin(end);
    std::reverse_iterator<S> rend(begin);
...
#else
    return find_last_or_default_if(
        std::forward<Iterable>(iterable), [&unary_predicate](const val_t<Iterator>& val) { return !unary_predicate(val); },
        std::forward<U>(default_value));
#endif
}
