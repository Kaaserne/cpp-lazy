...
struct cached_size_adaptor {
     ...
     * only useful for non sized iterables and iterables that are bidirectional (so forward/random access excluded). The following
     * iterables require a (eagerly)sized iterable:
     * - `lz::chunks`
     * - `lz::enumerate`
     * - `lz::exclude`
     * - `lz::interleaved`
