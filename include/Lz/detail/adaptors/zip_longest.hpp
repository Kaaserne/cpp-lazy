...
struct zip_longest_adaptor {
     ...
     * contains a tuple of `std::reference_wrapper`s to the elements itself if it is not empty. Contains a size() method if all
     ...
     * iterable. `lz::iter_decay` decays the iterable into a forward one and since forward iterators cannot go backward, its entire
     * size is therefore also not needed to create an iterator from its end() funciton. `lz::cache_size` however will traverse the
