...
/**
 * @brief Zips n containers and applies (simultaneously) the function given as argument, containing each of the containers' value
...
using select_iterable = filter_map_iterable<decltype(
    lz::zip(std::forward<Iterable>(std::declval<Iterable>()), std::forward<SelectorIterable>(std::declval<SelectorIterable>()))),
