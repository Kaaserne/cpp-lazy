 ...
 * input iterables. Contains a sentinel if one of the iterables contains a sentinel or if one of them is forward. If its input
 * iterables all have a .size() method, then this iterable will also have a .size() method. If the input iterable is exactly
 * bidirectional and not sized (like `lz::filter` for example), the entire sequence is traversed to get its end size (using
