 ...
 * `pair::first` is the element and `pair::second` is the count. The count is `std::size_t` and starts from 1. The input iterable
 * needs to be sorted first in order to count the duplicates. The iterator category is min(bidirectional, input iterator
 * category). Its end() function will return a sentinel if the input iterable is forward. This iterable does not contain a .size()
 * method. Example:
 * ```cpp
